...
import { FileText, MessageSquare, CheckCircle, ChevronLeft, Download, AlertCircle } from 'lucide-react';
export default function LawyerTaskDetail() {
    ...
    if (!mounted)
        return null;
    if (!taskDetail)
        return <div className="p-20 text-center animate-pulse text-indigo-500 font-black uppercase tracking-widest">Loading Manifest...</div>;
      ...
      <div className="max-w-7xl mx-auto px-8 py-12 grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-8 space-y-8">
           <section className="bg-slate-900 border border-slate-800 p-10 rounded-[2.5rem] shadow-2xl">
              ...
              <textarea value={draft} onChange={e => setDraft(e.target.value)} className="w-full h-[600px] bg-slate-950 border border-slate-800 p-10 rounded-3xl font-serif text-lg leading-relaxed shadow-inner"/>
        ...
        <aside className="lg:col-span-4 space-y-8">
           <section className="bg-slate-900 border border-slate-800 p-8 rounded-[2rem]">
              ...
              <div className="space-y-3">
                 {attachments.map(a => (<div key={a.id} className="flex items-center justify-between p-4 bg-slate-950 border border-slate-800 rounded-2xl group hover:border-indigo-500 transition-all">
           ...
           <section className="bg-slate-900 border border-slate-800 p-8 rounded-[2rem]">
              ...
              <div className="space-y-4 max-h-[300px] overflow-y-auto mb-6 pr-2">
                 {messages.map(m => (<div key={m.id} className={`p-4 rounded-2xl text-xs ${m.senderRole === 'CLIENT' ? 'bg-slate-950 text-indigo-400' : 'bg-indigo-600/10 text-indigo-300'}`}>
                       <p className="font-bold mb-1 uppercase text-[8px] opacity-50">{m.senderRole}</p>
                       {m.content}
                    </div>))}
              </div>
              <div className="space-y-3">
                 <textarea value={infoMsg} onChange={e => setInfoMsg(e.target.value)} className="w-full h-24 bg-slate-950 border border-slate-800 p-3 rounded-xl text-xs font-medium" placeholder="Request additional information..."/>
